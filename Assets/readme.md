...
8) **AddChain()**:
   - `GetInitialLayout()` — жадный старт по вершинам chain:
     - базовый порядок строится от уже размещённых соседей,
     - score кандидатов может учитывать эвристики; на текущих графах рабочий практический дефолт — `Cycle Closure Bias = on`, `Topology Expansion Bias = off`,
     - для `cycle-chain` стартовый score дополнительно держит под контролем `closure slack`: gap считается по реальным CS‑смещениям оставшихся рёбер цикла, а не по размеру room prefab; два открытых конца цикла не должны разойтись слишком далеко или схлопнуться слишком рано относительно оставшихся узлов; до поздней стадии предпочитается более широкий gap, а слишком близкие почти‑параллельные ветви от одного anchor штрафуются,
   ...
   - layout может быть перезапущен несколько раз с разными seed’ами (`layoutRetries`), так как основная случайность алгоритма находится именно в layout/SA.
9) **Кандидаты позиций**:
...
10) **Энергия** (`ComputeEnergy`) штрафует:
   ...
   - опционально `wall↔wall`, если включён `Disallow Wall-Wall Overlap`,
   - `D_tiles`: расстояние для рёбер, которые ещё не “в контакте” по CS.
   - при включённом topology‑режиме SA дополнительно штрафует шаги, которые тянут bridge‑ветки обратно в локальное скопление.
...
Таймаут `layoutTimeLimitSeconds` относится к **одной попытке layout generation**, а не к placement/stamping.

Логи:
- `[MapGraphLevelSolver] Layout attempt X/Y started ...` — старт очередной layout‑попытки с конкретным seed.
- `[MapGraphLevelSolver] Layout attempt X/Y failed ...` — попытка упала на layout/placement.
...
### 5.1) Что такое chain в этой реализации

В коде chain — это почти всегда **максимальный подграф степени `<= 2`**:
- `path-chain` (линейный путь),
- `cycle-chain` (простой цикл / simple cycle),
- singleton‑chain для изолированной вершины.

Это важно для понимания эвристик:
- внутри одной chain обычно нет произвольного ветвления,
- поэтому сложность чаще сидит не в “общем reorder узлов”, а:
  - в стыковке chain с уже размещёнными частями,
  - в замыкании циклов,
  - в выборе стартовой геометрии (`GetInitialLayout`),
  - в SA‑выходе из локальных минимумов.

Соответственно, эвристики вида bridge/articulation bias, layout retries и topology‑aware score оказываются полезнее, чем “тяжёлый” fail‑first как для общего CSP.
...
- **Wall↔Wall**: по умолчанию допускается; при включённом `Disallow Wall-Wall Overlap` считается нелегальным пересечением везде, кроме bite/carve‑клеток, которые всё равно вырезаются у коннектора при placement.
