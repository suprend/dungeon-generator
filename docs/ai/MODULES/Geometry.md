...
Важно: в placement’е carve‑mask не только “разрешает overlap”, но и реально модифицирует геометрию коннектора:
- стены на центральной и боковых дорожках удаляются,
- пол на центральной дорожке удаляется для `k < X` (в `door cell` сохраняется, чтобы избежать “дыр”),
- пол/стены на боковых клетках (`±tangent`) также удаляются.
